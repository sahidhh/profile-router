...
- `/profile off` / `/profile on` — kill switch for the whole router. While off,
  the `before_agent_start` handler returns early: no classification, model
  routing, thinking-level or toolset change, rule injection, or agent blocking —
  the turn runs as if the extension were absent. `off` takes effect immediately
  (not just on the next prompt): it restores any toolset the router had
  restricted, clears the active config so the `tool_call` agent-block hook goes
  inert, and sets the status line to `⏸ off`. `on` resumes routing on the
  following prompt. The flag **persists across sessions**: `off`/`on` write
  `{"enabled": bool}` to `.omp/routing-state.json` (project-local, gitignored,
  like `model-decisions.json`), and the state is reloaded once on the first
  prompt of each session — so a project left off comes back off after a restart.
  A missing or malformed file defaults to on. Distinct from `/profile clear`,
  which only unpins and leaves auto-classification running.
