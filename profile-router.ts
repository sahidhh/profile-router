/**
 * profile-router.ts — OMP extension
 * Install: ~/.omp/extensions/profile-router.ts   (global, auto-discovered)
 * Config:  .omp/bundles.json (project-local) → ~/.omp/bundles.json (fallback)
 *
 * Behavior:
 *  - Reclassifies EVERY prompt via keyword matching (no LLM call)
 *  - Injects merged profile rules into system prompt per-agent-run
 *  - Blocks disabled agents/tools via tool_call hook
 *  - Model changes require one-tap confirm; remembers answer per (from→to) pair
 *  - /profile command: status + manual override (sticky until cleared)
 *
 * TODO(VERIFY) markers = 20-min source-reading tasks against your installed OMP version.
 */

// TODO(VERIFY): OMP fork import path. Upstream Pi uses:
//   import type { ExtensionAPI } from "@mariozechner/pi-coding-agent";
// OMP docs show hooks at "@oh-my-pi/pi-coding-agent/extensibility/hooks" (HookAPI).
// Check node_modules or docs/extensions.md in your installed version.
import type { ExtensionAPI } from "@oh-my-pi/pi-coding-agent";
import * as fs from "node:fs";
import * as path from "node:path";
import * as os from "node:os";

// ---------- Types (mirror bundles.json schema) ----------

interface Profile {
  name: string;
  keywords: string[];          // classifier terms
  rules?: string[];            // injected into system prompt
  skills?: string[];           // union
  tools?: string[];            // union
  disabledAgents?: string[];   // INTERSECTION across matched profiles
  model?: string;              // single-value: highest score wins
  thinkingLevel?: string;      // single-value: highest score wins
}

interface Bundles {
  profiles: Profile[];         // declaration order = tiebreak order
  default?: Partial<Profile>;
}

interface MergedConfig {
  matched: { name: string; score: number }[];
  rules: string[];
  skills: string[];
  tools: string[];
  disabledAgents: string[];
  model?: string;
  thinkingLevel?: string;
}

// ---------- Config loading (project-local overrides global) ----------

function loadBundles(cwd: string): Bundles {
  const candidates = [
    path.join(cwd, ".omp", "bundles.json"),
    path.join(os.homedir(), ".omp", "bundles.json"),
  ];
  for (const p of candidates) {
    if (fs.existsSync(p)) {
      return JSON.parse(fs.readFileSync(p, "utf-8")) as Bundles;
    }
  }
  return { profiles: [] };
}

// ---------- Classification (port of promptbuilder.py matcher) ----------

function classify(prompt: string, bundles: Bundles): { profile: Profile; score: number }[] {
  const text = prompt.toLowerCase();
  const hits: { profile: Profile; score: number; order: number }[] = [];

  bundles.profiles.forEach((profile, order) => {
    let score = 0;
    for (const kw of profile.keywords) {
      // Word-boundary match beats naive substring ("fix" shouldn't hit "prefix").
      // TODO: replace with your promptbuilder.py scoring if it weights keywords.
      const re = new RegExp(`\\b${kw.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`);
      if (re.test(text)) score += 1;
    }
    if (score > 0) hits.push({ profile, score, order });
  });

  // Sort: score desc, then declaration order asc (your tiebreak rule)
  hits.sort((a, b) => b.score - a.score || a.order - b.order);
  return hits.map(({ profile, score }) => ({ profile, score }));
}

// ---------- Merge (your semantics: union / intersection / highest-score) ----------

function merge(matches: { profile: Profile; score: number }[], bundles: Bundles): MergedConfig {
  const cfg: MergedConfig = {
    matched: matches.map((m) => ({ name: m.profile.name, score: m.score })),
    rules: [],
    skills: [],
    tools: [],
    disabledAgents: [],
  };

  const union = (target: string[], src?: string[]) => {
    for (const item of src ?? []) if (!target.includes(item)) target.push(item);
  };

  // disabledAgents: intersection — any profile needing an agent keeps it enabled
  let disabled: Set<string> | null = null;
  for (const { profile } of matches) {
    union(cfg.rules, profile.rules);
    union(cfg.skills, profile.skills);
    union(cfg.tools, profile.tools);
    const d = new Set(profile.disabledAgents ?? []);
    disabled = disabled === null ? d : new Set([...disabled].filter((x) => d.has(x)));
  }
  cfg.disabledAgents = [...(disabled ?? [])];

  // Single-value fields: matches[0] is highest score + earliest declaration
  cfg.model = matches[0]?.profile.model ?? bundles.default?.model;
  cfg.thinkingLevel = matches[0]?.profile.thinkingLevel ?? bundles.default?.thinkingLevel;

  // Fallback to default profile when nothing matched
  if (matches.length === 0 && bundles.default) {
    union(cfg.rules, bundles.default.rules);
    union(cfg.skills, bundles.default.skills);
    union(cfg.tools, bundles.default.tools);
    cfg.disabledAgents = bundles.default.disabledAgents ?? [];
  }
  return cfg;
}

// ---------- Extension ----------

export default function (pi: ExtensionAPI) {
  let active: MergedConfig | null = null;
  let manualOverride: string | null = null;        // set via /profile <name>
  let currentModel: string | undefined;            // tracked, not assumed
  const modelDecisions = new Map<string, boolean>(); // "from→to" -> user's answer

  // ---- Every prompt: classify, merge, inject ----
  pi.on("before_agent_start", async (event: any, ctx: any) => {
    const cwd = process.cwd();
    const bundles = loadBundles(cwd);

    let matches = classify(event.prompt ?? event.userMessage ?? "", bundles);
    // TODO(VERIFY): exact field carrying the user prompt text on this event.

    if (manualOverride) {
      const p = bundles.profiles.find((x) => x.name === manualOverride);
      if (p) matches = [{ profile: p, score: Infinity }];
    }

    const next = merge(matches, bundles);
    const profileChanged =
      JSON.stringify(next.matched) !== JSON.stringify(active?.matched ?? []);
    active = next;

    // Status line: ALWAYS visible so misclassification is caught before damage
    ctx.ui.setStatus(
      "profile",
      next.matched.length
        ? `⚙ ${next.matched.map((m) => m.name).join("+")}${manualOverride ? " (manual)" : ""}`
        : "⚙ default",
    );

    // ---- Model routing: suggest + confirm, only on actual change ----
    if (next.model && next.model !== currentModel) {
      const key = `${currentModel ?? "?"}→${next.model}`;
      let approved = modelDecisions.get(key);
      if (approved === undefined) {
        approved = await ctx.ui.confirm(
          "Switch model?",
          `Profile "${next.matched[0]?.name ?? "default"}" suggests ${next.model} (current: ${currentModel ?? "unknown"})`,
        );
        modelDecisions.set(key, approved);
      }
      if (approved) {
        // TODO(VERIFY): the API for programmatic model switching mid-session.
        // Pi exposes model access on ctx (docs: "Access to models and API keys").
        // Likely candidates: ctx.model.set(...) / ctx.session.setModel(...).
        // Fallback if unavailable per-prompt: ctx.ui.notify suggesting /model.
        try {
          await ctx.model?.set?.(next.model);
          currentModel = next.model;
        } catch {
          ctx.ui.notify(`Run /model ${next.model} manually`, "warning");
        }
      }
    }

    // ---- Rules injection into system prompt for this agent run ----
    if (next.rules.length > 0) {
      return {
        systemPrompt:
          event.systemPrompt +
          "\n\n## Active Engineering Rules (" +
          (next.matched.map((m) => m.name).join("+") || "default") +
          ")\n" +
          next.rules.map((r) => `- ${r}`).join("\n"),
      };
    }
    // Silent when profile unchanged and no rules — zero UI noise
    void profileChanged;
  });

  // ---- Enforce disabledAgents ----
  pi.on("tool_call", async (event: any) => {
    if (!active) return;
    // TODO(VERIFY): how subagent invocations surface here. Likely toolName === "task"
    // with the agent name in event.input; adjust the predicate accordingly.
    const target = String(event.input?.agent ?? event.input?.subagent ?? "");
    if (event.toolName === "task" && active.disabledAgents.includes(target)) {
      return { block: true, reason: `Agent "${target}" disabled by profile ${active.matched.map((m) => m.name).join("+")}` };
    }
  });

  // ---- Manual override + status ----
  pi.registerCommand("profile", {
    description: "Show active profile, or override: /profile <name> | /profile clear",
    handler: async (args: string, ctx: any) => {
      const arg = (args ?? "").trim();
      if (arg === "clear") {
        manualOverride = null;
        ctx.ui.notify("Profile override cleared — auto-classification resumed", "success");
        return;
      }
      if (arg) {
        manualOverride = arg;
        ctx.ui.notify(`Profile pinned to "${arg}" until /profile clear`, "success");
        return;
      }
      ctx.ui.notify(
        active
          ? `Active: ${active.matched.map((m) => `${m.name}(${m.score})`).join(", ") || "default"}\n` +
            `Model: ${active.model ?? "unset"} | Disabled agents: ${active.disabledAgents.join(", ") || "none"}`
          : "No classification yet — send a prompt first",
        "info",
      );
    },
  });
}
